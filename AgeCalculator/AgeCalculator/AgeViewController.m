...
- (IBAction)birthdateSelected:(UIDatePicker *)sender
{
    ...
    // 7. We need to use the user's birthdate and today's date to calculate their age. Fortunately, we have a method already defined called "findAgeFromBirthdate" that does just that. How do we call it? ANS:  I am not sure how we call it I haven't learned to call a method.  I had to hire a tutor for this one.
    int age = [self findAgeFromBirthdate:birthdate];
...
- (void)configureCurrentDateLabel
{
    ...
    // 6. For the right side of the above instruction, can we just put "today" after the equals sign? What is that dateFormatter object we created earlier used for? Is it involved in the above instruction (look up the documentation for the NSDateFormatter class; I think we might want a "string" from a "date")?  ANS: Thank you Google for the answer.
    
    self.currentDateLabel.text = [_dateFormatter stringFromDate:today];
}
